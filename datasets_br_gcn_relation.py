...
from br_gcn_relation import BRGCNRelation
import statistics
import numpy as np
...
class Net(torch.nn.Module):
    def __init__(self, interim_channel=16, num_bases=4, num_nodes=data.num_nodes, num_edges=data.num_edges):
        super(Net, self).__init__()
        self.interim_channel=interim_channel
        self.num_bases=num_bases
        self.num_nodes=num_nodes
        self.num_edges=num_edges
        self.conv1 = BRGCNRelation(
            data.num_nodes, self.interim_channel, dataset.num_relations, self.num_bases, self.num_nodes, self.num_edges)
        self.conv2 = BRGCNRelation(
            self.interim_channel, dataset.num_classes, dataset.num_relations, self.num_bases, self.num_nodes, self.num_edges)
...
for trial in range(0, num_trials):
    for runs in range(0, num_runs):
        ...
        shutil.rmtree(path)
        dataType = 'Entities'  # Planetoid/TUDataset/Optm
        path = osp.join(osp.dirname(osp.realpath(__file__)), 'datasets', dataType)
